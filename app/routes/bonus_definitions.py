...
def list_bonus_definitions(
    ...
    if brand:
        q = q.filter(BonusDefinition.brand == brand)
...
def create_bonus_definition(payload: BonusDefinitionCreate, db: Session = Depends(get_db)):
    existing = (
...
def get_bonus_definition(bonus_definition_id: UUID, db: Session = Depends(get_db)):
...
def delete_bonus_definition(bonus_definition_id: UUID, db: Session = Depends(get_db)):
    obj = db.query(BonusDefinition).filter(BonusDefinition.id == bonus_definition_id).first()
    if not obj:
        raise HTTPException(status_code=404, detail="Bonus definition not found")
