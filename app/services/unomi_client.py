...
class UnomiClient:
    ...
    def search_profile_ids(
        self,
        *,
        property_name: str,
        property_value: str,
        limit: int = 5,
        sortby: str = "systemProperties.lastUpdated:desc",
    ) -> list[str]:
        """Find profile itemIds by a single profilePropertyCondition."""
        body = {
            "offset": 0,
        ...
        try:
            payload = self.request("POST", "/profiles/search", json_body=body)
        except UnomiClientError:
            return []
        ids: list[str] = []
        for item in (payload or {}).get("list") or []:
            if isinstance(item, dict):
                pid = item.get("itemId") or item.get("profileId")
                if pid:
                    ids.append(str(pid))
        return ids

    def resolve_canonical_profile_id(
        self,
        *,
        brand: str,
        email: str,
        ...
        """Best-effort canonical Unomi profile for an email (post-merge)."""
        norm_email = (email or "").strip().lower()
        if not norm_email or "@" not in norm_email:
            return fallback_profile_id
        scope_email = f"{brand}-{norm_email}".lower()
        seen: set[str] = set()
        ordered: list[str] = []
        for prop, value in (
            ("properties.scopeEmail", scope_email),
            ("properties.email", norm_email),
        ):
            for pid in self.search_profile_ids(property_name=prop, property_value=value):
                if pid not in seen:
                    seen.add(pid)
                    ordered.append(pid)
        return ordered[0] if ordered else fallback_profile_id
